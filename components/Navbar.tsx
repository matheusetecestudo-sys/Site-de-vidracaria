  ...
  return (
    <>
      <header className={`fixed top-0 w-full z-[100] transition-all duration-300 ${
        scrolled || isMenuOpen ? 'py-4 bg-white shadow-md' : 'py-6 bg-transparent'
      }`}>
        <div className="max-w-7xl mx-auto px-5 md:px-10">
          <div className="flex justify-between items-center h-10">
            
            {/* Logo perfeitamente alinhada à esquerda */}
            <button onClick={() => handleNav('home')} className="flex items-center gap-3 relative z-[110]">
              <div className="w-10 h-10 bg-[#00966d] rounded-xl flex items-center justify-center text-white font-black text-xl shadow-lg shadow-emerald-600/20">C</div>
              ...
              {navItems.map((item) => (
                <button 
                  ...
                  className={`text-sm font-bold transition-all ${
                    currentPage === item.id ? 'text-[#00966d]' : 'text-slate-500 hover:text-[#00966d]'
                  }`}
                >
                  {item.label}
                </button>
              ))}
              <a href={WHATSAPP_LINK} className="bg-[#00966d] text-white px-7 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-emerald-700 active:scale-95 transition-all">
                Orçamento
              </a>
            </nav>
      ...
      <div className={`fixed inset-0 bg-white z-[100] md:hidden transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col ${
        ...
        <div className="flex-1 flex flex-col justify-center items-center gap-10 px-10 pt-10">
          {navItems.map((item, idx) => (
            <button 
              ...
              } ${currentPage === item.id ? 'text-[#00966d]' : 'text-slate-900'}`}
          ...
          <a 
            href={WHATSAPP_LINK}
            className={`w-full mt-10 py-6 bg-[#00966d] text-white text-center rounded-[2rem] font-black text-lg uppercase tracking-widest shadow-xl active:scale-95 transition-all duration-300 ${
              isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            }`}
