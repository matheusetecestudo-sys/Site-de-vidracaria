    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-10">
          <div className="flex justify-between items-center h-10">
            ...
            <button onClick={() => handleNav('home')} className="flex items-center gap-3 relative z-[110]">
              <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black text-xl shadow-lg shadow-blue-600/20">C</div>
            ...
            <nav className="hidden md:flex items-center gap-8">
              {navItems.map((item) => (
                <button
                  ...
                  className={`text-sm font-bold transition-all ${currentPage === item.id ? 'text-blue-600' : 'text-slate-500 hover:text-blue-600'
                    }`}
                >
                  {item.label}
                </button>
              ))}
              <a href={WHATSAPP_LINK} className="bg-blue-600 text-white px-7 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-blue-700 active:scale-95 transition-all">
                Orçamento
              </a>
            </nav>
      ...
      <div className={`fixed inset-0 bg-white z-[100] md:hidden transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
        ...
        <div className="flex-1 flex flex-col justify-center items-center gap-10 px-10 pt-10">
          {navItems.map((item, idx) => (
            <button
              ...
              className={`text-4xl font-black tracking-tighter transition-all duration-300 ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                } ${currentPage === item.id ? 'text-[#2563eb]' : 'text-slate-900'}`}
          ...
          <a
            href={WHATSAPP_LINK}
            className={`w-full mt-10 py-6 bg-[#2563eb] text-white text-center rounded-[2rem] font-black text-lg uppercase tracking-widest shadow-xl active:scale-95 transition-all duration-300 ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
