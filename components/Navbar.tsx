  ...
  return (
    <>
      <header className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 ${scrolled ? 'bg-black/95 backdrop-blur-xl shadow-2xl' : 'bg-black/80 backdrop-blur-sm'
        ...
        <div className="container-precision flex justify-between items-center px-4 md:px-0 h-24 md:h-32">

          {/* Logo - Esquerda (Ocupando quase toda a altura do cabeçalho) */}
          <button
          ...
          >
            <img
              ...
              alt="Duno Vidraçaria"
              className="h-22 md:h-30 w-auto object-contain transition-all duration-500"
            />
          </button>

          {/* Grupo da Direita - Navegação e CTA */}
          <div className="flex items-center gap-6 lg:gap-10">
            ...
            <nav className="hidden lg:flex items-center gap-8">
              {navItems.map((item) => (
                <button
                  ...
                  className={`text-[10px] lg:text-[11px] font-bold uppercase tracking-[0.3em] transition-all relative py-2 group focus:outline-none ${currentPage === item.id ? 'text-brand-accent' : 'text-white/80 hover:text-brand-accent'
                    }`}
                >
                  {item.label}
            ...
            {/* Botão Orçamento - SOMENTE DESKTOP */}
            <a
              href={getWhatsAppLink("Navbar", "Orçamento Rápido")}
              target="_blank"
              rel="noopener noreferrer"
              className="hidden lg:flex items-center justify-center gap-2 px-8 h-12 btn-gold-premium text-black rounded-xl font-bold text-sm btn-interact shadow-xl shadow-black/20 focus:outline-none whitespace-nowrap leading-none"
            >
            ...
            {/* Menu Hambúrguer - SOMENTE MOBILE/TABLET */}
            <button
              onClick={() => setIsMenuOpen(!isMenuOpen)}
              className="lg:hidden w-12 h-12 flex items-center justify-center bg-black/50 backdrop-blur-md rounded-2xl border border-brand-accent/20 shadow-sm focus:outline-none touch-manipulation"
      ...
      < div className={`fixed top-0 right-0 h-screen w-full sm:w-[400px] bg-black z-[100] md:hidden transition-transform duration-500 ease-in-out transform-gpu shadow-2xl ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
        }`}>
        <div className="h-full flex flex-col pt-32 pb-12 px-10 items-center justify-center">
          <p className="text-[10px] font-black tracking-[0.4em] text-brand-accent uppercase mb-8 text-center">Navegação</p>
            ...
            {navItems.map((item, idx) => (
              <button
                ...
                className={`text-4xl font-black tracking-tighter text-center py-4 transition-all duration-500 touch-manipulation focus:outline-none ${isMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-[50px] opacity-0'
                  } ${currentPage === item.id ? 'text-brand-accent' : 'text-white'}`}
